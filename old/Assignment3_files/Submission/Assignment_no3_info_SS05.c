...
void asm_read()
{
	...
	uint8_t lbl_add[10] = {0,0,0,0};	//holds label address
	...
	while (lines != 0)
	{
		...
		if (opc == 0 || opc == 1)			//for load and store operation
		{
			opr1_s = strtok(NULL, ",");
			opr2_s = strtok(NULL, ",");
			opr3_s = strtok(NULL, " ");
		...
		else if (opc == 7 || opc == 8 || opc==13 || opc==14 || opc==17 || opc==18)	//for push, pop, jump immediate, jump register, increment, decrement operation
		{
			opr1_s = strtok(NULL, " ");
			if (opc == 13)				//for jump immediate instruction
			{
...
void display()
{
	...
	for (i = OS_MAX + 1, j = 1; i <= OS_MAX + 48; i++, j++)
	{
		printf("%03x    ", i);
		printf("%02x    ", MEM[i]);
		if (j % 6 == 0)
...
int lea(uint8_t baseaddr, uint8_t index, uint8_t wordsize)
{
