...
#define INST_LEN 25
#define STACK_MAX 3999    //sudheer
#define STACK_MIN 3800    //sudheer
...
int negation(int);
void flags(uint64_t);
void push(uint32_t);  //sudheer
uint32_t pop();   //sudheer
...
void input_cmdline(){
	...
	//=========================PUSH,POP Operations==================================//
	else if(strchr(userinst,'p')!=NULL){
		oper=strtok(userinst," ");
		tempstr= strtok(NULL," ");
...
void push(uint32_t data)  //sudheer
{
...
uint32_t pop(uint32_t* reg_num)   //sudheer
{
    //uint32_t data = 0;
