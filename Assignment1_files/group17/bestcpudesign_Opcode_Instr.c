...
uint8_t opcode = 0x00;//can be changed to avoid default Load operation
...
while(1)
{
    ...
    current_instr = ((uint32_t) opcode << 24);
    printf("The instruction that will be stored in Instruction memory is: 0x%x\n",current_instr);
    current_instr += ((uint32_t) reg_num << 20);
    printf("The instruction that will be stored in Instruction memory is: 0x%x\n",current_instr);
    current_instr += (off << 12);
    printf("The instruction that will be stored in Instruction memory is: 0x%x\n",current_instr);
    current_instr += loc2;
    printf("The instruction that will be stored in Instruction memory is: 0x%x\n",current_instr);
    ...
    case 0x00:
        ...
        break;
    case 0x01:
        printf("Store instruction is being performed. The corresponding opcode is 0x00\n");
        storereg(loc1,reg_ptr,mem_addr);
        break;
	}
