...
uint32_t* reg_ptr,inst_ptr,base_ptr;;
...
int loadreg(char* reg1,uint32_t* reg_num,int memory_addr )
{
	...
	*reg_num = 0x00000000;
	...
	instptr= instptr +4;
	printf("Register %s changed from: 0x%x to: 0x%x\n",reg1,old_reg,*reg_num);
	//display();
	return 0;
}
...
{
	int x,y=8;
	uint8_t old_mem,temp;
	old_mem =  mem[memory_addr];
	temp= *reg_num;
	printf("%x\n",temp);
	for(x=1;x<=4;x++)
		{
...
void display()
{
	...
	for(x=INST_LIMIT+1,y=1;x<MEMSIZE+1;x=x+4,y++){
		...
		printf("0x%x\t%x%x%x%x ",x,mem[x],mem[x+1],mem[x+2],mem[x+3]);
		if(y%6==0){
			printf("\n");
		}
	}
