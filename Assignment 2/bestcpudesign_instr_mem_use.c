...
void instruction_calc(uint8_t opcode)
{
	...
	current_instr = ((uint32_t) opcode << 24); //8 bit opcode
	    current_instr += ((uint32_t) reg_num << 20); //4 bit reg_num1
	    current_instr += ((uint32_t) reg_num << 16); //4 bit reg_num2
	    current_instr += (off << 4); //12 bit offset

	    current_instr += loc2; //4 bit memory address //scaling factor
