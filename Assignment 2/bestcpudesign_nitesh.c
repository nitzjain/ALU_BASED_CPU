...
int *loadreg(uint32_t* reg_num, int memory_addr );

int *storereg(uint32_t* reg_num, int memory_addr );
	...
	do{
		...
		case 0x00:
			...
			val2 = *src_p1+*src_p2*src3+src4;
						...
						temp = *loadreg(dest_p,val2);
						//*dest_p = *loadreg(dest_p,val2);
		...
		case 0x01:
			...
			temp = mem[instptr+2];
						...
						temp=*storereg(dest_p,val2);
//								*dest_p = *storereg(dest_p,val2);
...
*/
			...
			//call the load/store immediate instruction
			//*dest_p = (*fun_ptr_arr[4])(val1,val2);
...

{
	...
	else if ((opcode == 0x07) || (opcode == 0x08))
	{
		...
		current_instr += ((uint32_t) imm << 8); //4 bit reg_num2
